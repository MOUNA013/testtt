...
function Register() {
    grecaptcha.ready(function () {
        grecaptcha.execute('6Lfvq-opAAAAAL0VtGCzIPU8FI0ZNHN5sDcbJzGP', { action: 'submit' }).then(function (token) {
            ...
            if (!AcceptTerms) {
                document.getElementById("AcceptTermsAlert").style.display = 'flex'; // Display message about terms
                registerBtn.classList.add('disabled');
                registerBtn.disabled = true;
                return; // Stop further execution if terms not accepted
            } else {
                registerBtn.classList.remove('disabled');
                registerBtn.disabled = false;
                document.getElementById("AcceptTermsAlert").style.display = 'none'; // Hide alert
            }
